...
var Renderer = {
  ...
  parseEntities: function(text, entities) {
    ...
    for (var i = 0, len = orderedEntities.length; i < len; ++i) {
      ...
      if (entity.type === 'mention') {
        ...
        link.addEventListener('mouseover', function() { Renderer.expandLink(this, entity.expanded_url); }, false);

      } else {
        elements.appendChild(Renderer.makeText(text.substring(indices[0], indices[1])));
      }
    ...
    elements.appendChild(Renderer.makeText(text.substring(totalInc, text.length )));
...
function openTab(tabUrl) {
  ...
  tabUrl = tabUrl.replace(/[^a-zA-Z0-9]+$/, "");
